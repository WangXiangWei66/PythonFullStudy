...
class TrainSpider(object):
    ...
    def search_ticket(self):
        ...
        spantag=driver.find_element_by_id('qd_closeDefaultWarningWindowDialog_id')
        spantag.click()
            ...
            for train in trains:
                ...
                if train_no in self.trains:
                    ...
                    for seat_type in seat_types:  # 遍历席位的列表
                        ...
                        elif seat_type == '1':  # 说明是硬座
                            count = infos[15]  # 索引为15的是硬座
                            if count.isdigit() or count == '有':
                                is_flag = True
                                break

                        elif seat_type == '3':  # 说明是硬卧
                            count = infos[13]  # 索引为13的是硬卧
                            if count.isdigit() or count == '有':
                                is_flag = True
                                break

                        elif seat_type == '4':  # 说明是软卧
                            count = infos[11]  # 索引为13的是软卧
                            if count.isdigit() or count == '有':
                                is_flag = True
                                break

                        elif seat_type == '6':  # 说明是高级软卧
                            count = infos[10]  # 索引为13的是高级软卧
                            if count.isdigit() or count == '有':
                                is_flag = True
                                break
                    # 判断是否有余票
...
#启动爬虫程序
def start():
              # O 表示的是二等座,M 代表是一等座  ，硬卧是  3 ， 硬座是1 ， 软卧是 4
             # 出发地：北京， 到达站：长沙，出发时间：2022-01-20 ，
             # Z35，硬卧， Z161 硬座或硬卧 ，乘车人是杨淑娟
    spider=TrainSpider('北京','长沙','2022-05-23', {'Z35': ['3'], 'Z161':['1','3']},['杨淑娟','杨子民'])
