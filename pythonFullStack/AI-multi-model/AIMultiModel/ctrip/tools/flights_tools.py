from datetime import datetime, date  # 处理日期时间
from sqlite3 import connect  # 操作SQLite数据库
...
def fetch_user_flight_information(config: RunnableConfig) -> List[Dict]:
    ...
    SELECT
            ...
            FROM 
            tickets t 
            JOIN ticket_flights tf ON f.ticket_no = tf.ticket_no
            JOIN flights f ON tf.flight_id = f.flight_id
            JOIN boarding_passes bp ON bp.ticket_no = t.ticket_np AND bp.flight_no = f.flight_no
            WHERE
            t.passenger_id = ?
            """
...
def update_ticket_to_new_flight(
    ...
    # 查询新航班信息
    cursor.execute(
        "SELECT departure_airport, arrival_airport, scheduled_departure FROM flights WHERE flisght_id = ?",
        (new_flight_id,),
    )
    ...
    if not current_ticket:
        cursor.close()
        conn.close()
        return f"当前登录的乘客ID 为 {passenger_id}, 不是机票{ticket_no}的拥有者。"
